    ...
    for profile_type, name, fr in zip([new_quiet_profiles, new_shock_reverse_other_profiles, shocks_78_18], names, frs):
        ...
        for frame in fr:
            ...
            for profile in profile_type:
                a, b, c = np.where(f['new_final_labels'][frame[0]:frame[1], x[0]:x[1], y[0]:y[1]] == profile)
                a_final += list(a)
                b_final += list(b)
                c_final += list(c)
            ...
            fo = h5py.File(write_path / 'pixel_indices_{}_frame_{}_{}_x_{}_{}_y_{}_{}_total_{}.h5'.format(name, frame[0], frame[1], x[0], x[1], y[0], y[1], a_final.size), 'w')
            ...
            ca_k.dat[0, 0, :, ick, 0] = data_3950[frame[0]:frame[1], 0, :-1, x[0]:x[1], y[0]:y[1]][a_final, :, b_final, c_final].T / cont[0]

            ca_k.dat[0, 0, :, -1, 0] = data_3950[frame[0]:frame[1], 0, :, x[0]:x[1], y[0]:y[1]][a_final, -1, b_final, c_final].T / cont[0]

            fe_1.dat[0, 0, :, ife, 0] = data_6173[frame[0]:frame[1], 0, :, x[0]:x[1], y[0]:y[1]][a_final, :, b_final, c_final].T * 1e3 / cont[1]

            fe_1.dat[0, 0, :, ife, 0] *= correction_factor[1]

            ca_8.dat[0,0,:,ic8, 0] = data_8542[frame[0]:frame[1], 0, :, x[0]:x[1], y[0]:y[1]][a_final, :, b_final, c_final].T * 1e3 / cont[2]
            ...
            write_filename = write_path / 'wholedata_{}_frame_{}_{}_x_{}_{}_y_{}_{}_total_{}.nc'.format(name, frame[0], frame[1], x[0], x[1], y[0], y[1], a_final.size)
            ...
            write_filename = write_path / 'wholedata_{}_initial_atmos_frame_{}_{}_x_{}_{}_y_{}_{}_total_{}.nc'.format(name, frame[0], frame[1], x[0], x[1], y[0], y[1], a_final.size)
...
if __name__ == '__main__':
    ...
    fov_list = [([486, 536], [974, 1024])]

    # fov_list = [([915, 965], [1072, 1122])]

    # fov_list = [([640, 690], [1000, 1050])]
    ...
    for fov in fov_list:
        ...
        quiet_frames_list = [[0, 25], [25, 50], [50, 75], [75, 100]]
        shock_reverse_other_frames_list = [[0, 25], [25, 50], [50, 75], [75, 100]]
        shocks_78_18_frames_list = [[0, 100]]
        make_files(
            frs=(
                quiet_frames_list,
