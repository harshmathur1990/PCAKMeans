...
def get_all_profiles_and_atmos():
    subfoldername='plots'
    base_path = Path('/home/harsh/OsloAnalysis/new_kmeans/wholedata_inversions/fov_662_712_708_758')
    subfolder = base_path / subfoldername
    output_prof_name = subfolder / 'wholedata_shock_profiles_rps_frame_0_21_x_662_712_y_708_758_total_11695_cycle_1_t_6_vl_5_vt_4_profs.nc'
    input_prof_name = base_path / 'wholedata_shock_profiles_rps_frame_0_21_x_662_712_y_708_758_total_11695.nc'
    output_atmos_name = subfolder / 'wholedata_shock_profiles_rps_frame_0_21_x_662_712_y_708_758_total_11695_cycle_1_t_6_vl_5_vt_4_atmos.nc'
    ...
    profiles = np.array(
        [
            0, 500, 1000, 3000, 5000, 7000, 9000, 11000, 11690
        ]
    )
    ...
    cont_arr = (
        input_prof['profiles'][0, 0, profiles, :, 0][:, 36],
        output_prof['profiles'][0, 0, profiles, :, 0][:, 36]
    )
    ...
    row_1.append(
        (
            input_prof['profiles'][0, 0, profiles, :, 0][:, wave_ind[0]],
            output_prof['profiles'][0, 0, profiles, :, 0][:, wave_ind[0]]
        )
    )

    row_1.append(
        output_atmos['temp'][0, 0, profiles]
    )
    ...
    row_2.append(
        (
            input_prof['profiles'][0, 0, profiles, :, 0][:, wave_ind[1]],
            output_prof['profiles'][0, 0, profiles, :, 0][:, wave_ind[1]]
        )
    )

    row_2.append(
        output_atmos['vlos'][0, 0, profiles] / 1e5
    )
    ...
    row_3.append(
        (
            input_prof['profiles'][0, 0, profiles, :, 0][:, wave_ind[2]],
            output_prof['profiles'][0, 0, profiles, :, 0][:, wave_ind[2]]
        )
    )

    row_3.append(
        output_atmos['vturb'][0, 0, profiles] / 1e5
    )
