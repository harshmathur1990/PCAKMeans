...
cont_array = np.ones(64)

cont_array[0:30] = cont[0]
cont_array[30:50] = cont[2]
cont_array[50:64] = cont[1]
...
def get_fov():
 ...
 #   )
    ...
    base_path = Path(
        '/home/harsh/OsloAnalysis/new_kmeans/wholedata_inversions/fov_more_frames'
    )
    ...
    for frames, name in zip(frs, frs_names):
        ...
        for frame in frames:
            input_profile_filepath = get_exact_filename(
                base_path,
                'wholedata_{}_profiles_rps_frame_{}_{}_more_frames_for_fov'.format(
            ...
            pixel_file = get_exact_filename(
                base_path,
                'pixel_indices_{}_profiles_rps_frame_{}_{}_more_frames_for_fov'.format(
                    name, frame[0], frame[1]
                ),
                '.h5'
            )
...
def plot_fov_parameter_variation(
    ...
    atmosparam_file = output_path / 'consolidated_results_velocity_calibrated_fov_more_frames.h5'
...
if __name__ == '__main__':
    ...
    base_path = Path(
        '/home/harsh/OsloAnalysis/new_kmeans/wholedata_inversions/fov_more_frames/plots/'
    )
    plot_fov_parameter_variation(
        animation_path=base_path / 'inversion_map_fov_more_frames.mp4'
    )
