...
fig, axs, obp, obs, inp, ins, temp, vlos, vturb = None, None, None, None, None, None, None, None, None
...
def do_work(index):
    ...
    try:
        ...
        if not fig:
            ...
            obp.set_ydata(observed['profiles'][0, 0, index, :, 0][indices])
            obs.set_offsets(np.c_[observed['wav'][indices], observed['profiles'][0, 0, index, :, 0][indices]])
            inp.set_ydata(profiles[0]['profiles'][0, 0, index, :-1, 0])
            ins.set_offsets(np.c_[observed['wav'][:-1], profiles[0]['profiles'][0, 0, index, :-1, 0]])
            temp.set_ydata(atmos[0]['temp'][0][0][index])
            vlos.set_ydata((atmos[0]['vlos'][0][0][index] - calib_velocity)/ 1e5)
            vturb.set_ydata(atmos[0]['vturb'][0][0][index] / 1e5)
