...
names = ['quiet_profiles_rps', 'shock_profiles_rps', 'shocks_78_18_profiles_rps', 'reverse_shock_profiles_rps', 'other_emission_profiles_rps']
write_path = None
x = None
y = None
frames = None
...
def make_files():
    ...
    for profile_type, name in zip([new_quiet_profiles, new_shock_profiles, shocks_78_18, new_reverse_shock_profiles, new_other_emission_profiles], names):
        ...
        fo = h5py.File(write_path / 'pixel_indices_{}_frame_{}_{}_x_{}_{}_y_{}_{}_total_{}.h5'.format(name, frames[0], frames[1], x[0], x[1], y[0], y[1], a_final.size), 'w')
        ...
        ca_k.dat[0, 0, :, ick, 0] = data_3950[frames[0]:frames[1], 0, :-1, x[0]:x[1], y[0]:y[1]][a_final, :, b_final, c_final].T / cont[0]

        ca_k.dat[0, 0, :, -1, 0] = data_3950[frames[0]:frames[1], 0, :, x[0]:x[1], y[0]:y[1]][a_final, -1, b_final, c_final].T / cont[0]

        fe_1.dat[0, 0, :, ife, 0] = data_6173[frames[0]:frames[1], 0, :, x[0]:x[1], y[0]:y[1]][a_final, :, b_final, c_final].T * 1e3 / cont[1]

        fe_1.dat[0, 0, :, ife, 0] *= correction_factor[1]

        ca_8.dat[0,0,:,ic8, 0] = data_8542[frames[0]:frames[1], 0, :, x[0]:x[1], y[0]:y[1]][a_final, :, b_final, c_final].T * 1e3 / cont[2]
        ...
        write_filename = write_path / 'wholedata_{}_frame_{}_{}_x_{}_{}_y_{}_{}_total_{}.nc'.format(name, frames[0], frames[1], x[0], x[1], y[0], y[1], a_final.size)
        ...
        write_filename = write_path / 'wholedata_{}_initial_atmos_frame_{}_{}_x_{}_{}_y_{}_{}_total_{}.nc'.format(name, frames[0], frames[1], x[0], x[1], y[0], y[1], a_final.size)
...
if __name__ == '__main__':
    ...
    frames_list = [[0, 21], [21, 42], [42, 63], [63, 84], [84, 100]]

    fov_list = [([662, 712], [708, 758]), ([520, 570], [715, 765]), ([486, 536], [974, 1024])]
    ...
    for fov in fov_list:
        ...
        write_path = base_path / 'new_kmeans/wholedata_inversions/fov_{}_{}_{}_{}/'.format(
            x[0], x[1], y[0], y[1]
        )
